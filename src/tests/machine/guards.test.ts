...
describe("machine/GameMachine", () => { 
  describe("join", () => { 
    ...
    it("should let a player join", () => { 
      expect(machine.send(GameModel.events.join("1", "1")).changed
        ).toBe(true)
        expect(machine.state.context.players).toHaveLength(1)
        expect(machine.send(GameModel.events.join("2", "2")).changed
        ).toBe(true)
        expect(machine.state.context.players).toHaveLength(2)

    })

    it("should not let me join the game twice", () => { 
      expect(machine.send(GameModel.events.join("1", "1")).changed
        ).toBe(true)
      expect(
        machine.send(GameModel.events.join("1", "1")).changed
        ).toBe(false)
    })
  ...
  describe("dropToken", () => { 
    ...
    it("should let me drop a token", () => { 
      ...
      expect(machine.state.value).toEqual(GameStates.PLAY)
    ...
    it("should make me win ", () => { 
      ...
      expect(machine.state.value).toEqual(GameStates.VICTORY)
